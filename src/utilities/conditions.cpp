                      ...
                      OptionList& optionlist)
{
    ...
    if (condition[0] == '(')
    {
        ...
        nestedcond = make_shared<nemesis::Condt>(
        ...
        if (backB + 2 < condition.length())
        {
            ...
            next = make_shared<nemesis::Condt>(outHouse.substr(1),
                                               format,
                                               behaviorFile,
                                               original,
                                               multiOption,
    ...
    else
    {
        ...
        if (c_and == NOT_FOUND)
        {
            ...
            else
            {
                ...
                singleCondition(condition.substr(0, c_or),
                                ...
                                isMaster,
                                optionlist);
                next = make_shared<nemesis::Condt>(condition.substr(c_or + 1),
                                                   format,
                                                   behaviorFile,
                                                   original,
                                                   multiOption,
        ...
        else if (c_or == NOT_FOUND)
        {
            singleCondition(condition.substr(0, c_and),
                            ...
                            isMaster,
                            optionlist);
            next = make_shared<nemesis::Condt>(condition.substr(c_and + 1),
                                               format,
                                               behaviorFile,
                                               original,
                                               multiOption,
                    ...
                    if ((*optioncondt)[2].length() == 0)
                    {
                        if (animthrinfo.animMulti == -1 || animthrinfo.multiOption != format)
                        {
                            ...
                            while (animMulti < a_size)
                            {
                                ...
                                const auto& pick = opt.find((*optioncondt)[3]);
        ...
        else
        {
            ...
            if (multi)
            {
                ...
                for (size_t i = 0; i < animMulti; ++i)
                {
                    ...
                    const auto& pick = opt.find((*optioncondt)[2]);
                    result           = isNot ? pick == opt.end() : pick != opt.end();
...
void nemesis::Condt::specialCondition(string condition,
                                      string format,
                                      string behaviorFile,
                                      string multiOption,
                                      int numline,
...
void nemesis::Condt::conditionProcess(string condition,
                                      string format,
                                      string behaviorFile,
                                      string multiOption,
                                      int numline,
...
nemesis::MultiChoice::MultiChoice(string cond,
                                  string format,
                                  string behaviorFile,
                                  string multiOption,
                                  int numline,
                                  ...
                                  size_t posB)
{
    if (cond.length() > 0)
    {
        condition = make_shared<nemesis::Condt>(
...
void nemesis::Condt::singleCondition(string condition,
                                     string format,
                                     string behaviorFile,
                                     string multiOption,
                                     int numline,
    ...
    else
    {
        ...
        if (addOnPtr->find(addinfo.header) != addOnPtr->end() && (*addOnPtr)[addinfo.header].size() > 0)
        {
                ...
                && (*groupAdditionPtr)[addinfo.addition].size() > 0)
            {
                ...
                {
                    ErrorMessage(1148,
                                 curcond.ig ? curcond.fmt + "_group" : curcond.fmt,
                                 curcond.bhvfile,
                                 curcond.nm,
                                 original);
                }
