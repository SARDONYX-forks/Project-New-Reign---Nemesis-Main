...
void InsertMessageParam(std::string& message, const std::string& newinput,
                        const std::string& replacement);
void InsertMessageParam(std::wstring& message, const std::wstring& newinput,
                        const std::wstring& replacement);

void AdditionalInput(std::string& message, int counter,
...
inline void AdditionalInput(std::string& message, int counter,
                            const std::wstring& input, other... rest) {
...
template <typename... other>
inline void AdditionalInput(std::string& message, int counter,
                            const std::wstring_view& input, other... rest) {
  AdditionalInput(message, counter, input);
  AdditionalInput(message, counter + 1, rest...);
}
...
template <typename... other>
inline void AdditionalInput(std::string& message, int counter,
                            const std::filesystem::path& input, other... rest) {
  AdditionalInput(message, counter, input);
  AdditionalInput(message, counter + 1, rest...);
}
...
template <typename... other>
inline void AdditionalInput(std::string& message, int counter,
                            const nemesis::Line& input, other... rest) {
  AdditionalInput(message, counter, input);
  AdditionalInput(message, counter + 1, rest...);
}
...
void AdditionalInput(std::wstring& message, int counter,
                     const std::wstring& input);
void AdditionalInput(std::wstring& message, int counter,
                     const std::wstring_view& input);
...
template <typename... other>
inline void AdditionalInput(std::wstring& message, int counter,
                            const std::string& input, other... rest) {
  AdditionalInput(message, counter, input);
  AdditionalInput(message, counter + 1, rest...);
}
...
template <typename... other>
inline void AdditionalInput(std::wstring& message, int counter,
                            const std::string_view& input, other... rest) {
  AdditionalInput(message, counter, input);
  AdditionalInput(message, counter + 1, rest...);
}
...
inline void ErrorMessage(int errorcode, other... rest) {
  ...
  std::wstring errormsg =
      L"ERROR(" + std::to_wstring(errorcode) + L"): " + DMLogError(errorcode);
  std::string englog =
      "ERROR(" + std::to_string(errorcode) + "): " + EngLogError(errorcode);

  if (DMLogError(errorcode).length() == 0) {
    errormsg =
        L"CRITICAL ERROR: Error code not found. Unable to diagnose problem. "
        L"Please re-install Nemesis";
    interMsg(errormsg + L"\n");
    return;
  }
...
inline void WarningMessage(int warningcode, other... rest) {
  ...
  std::wstring warninmsg = L"WARNING(" + std::to_wstring(warningcode) + L"): " +
                           DMLogWarning(warningcode);
  std::string englog = "WARNING(" + std::to_string(warningcode) +
                       "): " + EngLogWarning(warningcode);
