...
// NOTE: If static variables are uninitialized at compile time, link errors will occur, so initialize them with nullptr.
...
void AnimationsConvert::convert(std::string_view filePath, const hkPackFormat& pkFormat) const
{
    ...
    if (!istream.isOk()) throw std::runtime_error("An error occured while loading this file.");
    ...
    else
    {
        if (root.m_object != nullptr)
        {
            ...
            if (!ostream.isOk()) throw std::runtime_error("An error occured while saving this file.");

            res = hkSerializeUtilSave(pkFormat, root, ostream, flags, packFileOptions);

            if (res)
            {
                throw std::runtime_error(std::string("Failed to save file: ") + std::string(filePath));
            }
        }
        else
            throw std::runtime_error(std::string("Failed to load file: ") + std::string(filePath));
    }
    if (resource != nullptr) resource->removeReference();
