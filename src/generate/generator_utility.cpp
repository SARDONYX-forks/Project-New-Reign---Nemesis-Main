...
string behaviorLineChooser(const string& originalline,
                           const unordered_map<string, string>& chosenLines,
...
vector<unique_ptr<registerAnimation>> openFile(TemplateInfo* behaviortemplate, const NemesisInfo* nemesisInfo)
{
    ...
    for (auto& behaviorGroup : behaviortemplate->grouplist)
    {
        ...
        if (pos != NOT_FOUND && nextpos != NOT_FOUND && nextpos < pos)
            animPath.insert(wstring(path.substr(nextpos, pos - nextpos)) + L'\\');
    ...
    try
    {
        for (auto& file : filelist)
        {
            ...
            else if (nemesis::iequals(curfile.extension().string(), ".txt"))
            {
                ...
                {
                    if (directory.stem().wstring().find(L"~") != NOT_FOUND && file.length() > 0
                        && file[0] != L'$')
                    {
                        if (isChecked.find(path) == isChecked.end())
                        {
