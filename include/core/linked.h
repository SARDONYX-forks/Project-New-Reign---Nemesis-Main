...
struct Linked {
 ...
 public:
  ...
  Linked(const _Ty& value) { raw = std::make_shared<_Ty>(value); }

  void SetRaw(const _Ty& value) {
    if (raw)
      std::runtime_error(
          "Cannot set new raw value because current raw value is not nullptr");

    raw = std::make_shared<_Ty>(value);
  }

  void AddCondition(const nemesis::ConditionInfo& conditioninfo,
                    const nemesis::File& file) {
    conditions.emplace_back(
        std::make_shared<LinkedCondition>(conditioninfo, file));
  }
  ...
  LinkedCondition& GetFrontCondition() { return *conditions.front(); }

  LinkedCondition& GetLastCondition() { return *conditions.back(); }

  LinkedCondition& GetCondition(size_t index) { return *conditions[index]; }

  size_t GetConditionSize() const noexcept { return conditions.size(); }

  _Ty* GetRawPtr() { return raw.get(); }

  const _Ty* GetRawPtr() const { return raw.get(); }

  bool HasCondition() const noexcept { return !conditions.empty(); }
  ...
  virtual std::string RawToString() const {
    ...
    if constexpr (std::is_base_of_v<std::string, _Ty>)
      return static_cast<std::string>(*raw);

    throw std::runtime_error(
        "Invalid raw to string return value, use override to get a valid "
        "return value");
  }
    ...
    for (auto& condition : conditions) {
      ...
      switch (condition->GetType()) {
        case nemesis::CondType::FOREACH: {
          ...
          for (auto cur = scopeinfo.ExeQuery(*condition->GetCondition());
               cur && !cur->IsEnd(); cur->Next()) {
  ...
  virtual void SetRawConditionedDataTo(VecNstr& lines) const {
    lines.emplace_back(RawToString() + nemesis::syntax::Spaces() +
  ...
  virtual void GetRawData(VecNstr& lines) const {
    ...
    for (auto& condition : conditions) {
      switch (condition->GetType()) {
        ...
        case nemesis::CondType::FOREACH: {
          lines.emplace_back(
              nemesis::syntax::ForEach(condition->GetExpression()));
    ...
    for (auto& cond : modlist) {
      ...
      nemesis::Line mline =
          mod_lines.empty() ? nemesis::syntax::DeleteLine() : mod_lines.front();
