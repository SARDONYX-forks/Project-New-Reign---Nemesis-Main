...
struct CompileState {
 private:
  ...
  UMap<std::string, Vec<Vec<const nemesis::AnimationRequest*>>>
      ChildRequestList;
  const nemesis::AnimationRequestRepository& Repository;

  UMap<const nemesis::AnimationRequest*,
       UMap<std::string, Vec<const nemesis::TemplateOption*>>>
      CurrentRequestOption;
  UMap<std::string, Vec<const nemesis::TemplateOption*>> CurrentOption;

  UMap<const nemesis::AnimationRequest*,
       UMap<std::string, Vec<const std::string*>>>
      CurrentRequestMap;
 ...
 public:
  ...
  const nemesis::AnimationRequest* GetFirstRequest(
      const std::string& group) const;
  const nemesis::AnimationRequest* GetLastRequest(
      const std::string& group) const;
  const nemesis::AnimationRequest* GetNextRequest(
      const std::string& group) const;
  const nemesis::AnimationRequest* GetBackRequest(
      const std::string& group) const;

  void QueueCurrentRequest(const std::string& group,
                           const nemesis::AnimationRequest* request);
  void DequeCurrentRequest(const std::string& group);
  const nemesis::AnimationRequest* GetCurrentRequest(
      const std::string& group) const;

  void QueueChildRequestList(
      const std::string& group,
      const Vec<const nemesis::AnimationRequest*>& requests);
  void DequeChildRequestList(const std::string& group);
  const Vec<const nemesis::AnimationRequest*>& GetChildRequestList(
      const std::string& group) const;
  ...
  void DequeCurrentRequestMapValue(const nemesis::AnimationRequest* request,
                                   const std::string& key);
  const std::string* GetCurrentRequestMapValue(
      const nemesis::AnimationRequest* request, const std::string& key) const;
  ...
  const nemesis::AnimationRequestCollection& GetRequests(
  ...
  void DequeueRequestOption(const nemesis::AnimationRequest* request,
                            const std::string& option_name);
  const nemesis::TemplateOption* GetCurrentRequestOption(
      const nemesis::AnimationRequest* request,
      const std::string& option_name) const;

  void QueueOption(const std::string& option_name,
                   const nemesis::TemplateOption* option);
  void DequeueOption(const std::string& option_name);
  const nemesis::TemplateOption* GetCurrentOption(
      const std::string& option_name) const;
  ...
  void SetCurrentSubTemplateRequest(
      const nemesis::SubTemplateRequest& request) noexcept;
  const nemesis::SubTemplateRequest* GetCurrentSubTemplateRequest()
      const noexcept;
